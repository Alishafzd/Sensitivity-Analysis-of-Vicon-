function  Mat2 = EulerAngleTest1(Mat,Static0)
    ...
    for h=1:length(LR)     %Left,Right
        if isfield(Mat.ThiCo,(LR{h}))
            for t=1:size(Mat.ThiCo.(LR{h}),2)
                if ~isempty(Mat.ThiCo.(LR{h})(t).Org)
                    if j==1
                        R=[Static0.(Co{j}).x;
                            Static0.(Co{j}).y;
                            Static0.(Co{j}).z];
                ...
                elseif t==size(Mat.ThiCo.(LR{h}),2)
                    Mat2.(Co{j}).(LR{h})(t).(xyz{1})=[];
                end
            end
        end
    end
    ...
    if isfield(Mat2.FootCo,(LR{h}))
        if ~isempty(Mat2.FootCo.(LR{h}))
            for j=1:size(Mat2.FootCo.(LR{h}),2)
                if ~isempty(Mat.ThiCo.(LR{h})(j).Org)
                    temp=Mat.Trajectory.(LR{h})(j).LASI(:,1);
                    for f=1:size(Mat2.FootCo.(LR{h})(j).x,1)
                        ...
                        eul2(f,3)=atan2d(-R(2,1),-R(2,2));
                        clear R R1 R2
                        %                             Z=Mat2.FootCo.(LR{h})(j).z(f,1:2);
                        %                             FPA(f)=abs(acosd(dot(Z,[1 0])/norm(Z)));
                        %                             if h==1
                        %                                 if temp(end)>temp(1)
                        %                                     if Z(2)>0
                        %                                         FPA(f)=-FPA(f);
                    ...
                    %                     Mat2.EulFootProgressAngle.(LR{h})(j).D=[zeros(size(FPA,2),2) FPA'];
                    clear eul1 eul2 FPA
                elseif j==size(Mat.ThiCo.(LR{h}),2)
                   Mat2.EulFootProgressAngle.(LR{h})(j).D=[];
                end
            end
        end
    end
